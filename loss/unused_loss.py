...
class DFLoss(nn.Module):
    ...
    def forward(self, pred_dist, target):
        """
        Args:
            pred_dist: Predicted distribution [batch_size, 4, reg_max]
            target: Target values [batch_size, 4*reg_max] from bbox2dist
        Returns:
            loss: DFL loss value
        """
        batch_size = pred_dist.size(0)
        
        # Reshape target to match pred_dist shape [batch_size, 4, reg_max]
        target = target.view(batch_size, 4, self.reg_max)
        
        # Compute loss using cross entropy
        loss = F.cross_entropy(
            pred_dist.view(-1, self.reg_max),
            target.view(-1, self.reg_max),
            reduction='mean'
        )
