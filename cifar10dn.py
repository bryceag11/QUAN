...
from quaternion.conv import QConv, QConv2D, QDense
from quaternion.qactivation import  QPReLU, QREReLU, QReLU
import tqdm
import signal
import sys
import gc
...
class MetricsLogger:
    ...
    def save(self, filename='Q34_quat.json'):
        """Save metrics to JSON file"""
        with open(self.save_dir / filename, 'w') as f:
            json.dump(self.metrics, f)
    
    def load(self, filename='Q34_quat.json'):
        """Load metrics from JSON file"""
        with open(self.save_dir / filename, 'r') as f:
            self.metrics = json.load(f)
    
    def plot(self, save_path='Q34_quat.png'):
...
class QuaternionResNet(nn.Module):
    ...
    def __init__(self, num_classes=10, mapping_type='luminance'):
        ...
        self.conv2_x = self._make_layer(16, 16, blocks=3, stride=1)
        
        # Conv3_x block - 4 residual blocks (32 channels)
        self.conv3_x = self._make_layer(16, 32, blocks=4, stride=2)
        
        # Conv4_x block - 6 residual blocks (64 channels)
        self.conv4_x = self._make_layer(32, 64, blocks=6, stride=2)
        
        # Conv5_x block - 3 residual blocks (128 channels)
        self.conv5_x = self._make_layer(64, 128, blocks=3, stride=2)
        ...
        self.fc = QDense(128, num_classes * 4)
...
class QResNet34(nn.Module):
    ...
    def __init__(self, num_classes=10, mapping_type='raw_normalized'):
        ...
        self.dropout_rates = {
            'initial': [0.1, 0.15, 0.2, 0.2, .25],  # [block2, block3, block4, block5, classifier]
            'increment': 0.05  # Amount to increase after each LR drop
        }
        self.current_rates = self.dropout_rates['initial'].copy()
        ...
        self.conv5_x = self._make_layer(256, 256, 3, 2,  dropout_idx=3)
...
class QuaternionCIFAR10(nn.Module):
    ...
    def __init__(self, mapping_type='luminance'):
        ...
        self.initial_block = nn.Sequential(
            QConv2D(in_channels=3, out_channels=32, kernel_size=3, padding=1),
            IQBN(32),
            nn.ReLU(),
        )
        ...
        self.block2 = nn.Sequential(
            QConv2D(in_channels=32, out_channels=64, kernel_size=3, stride=2, padding=1),
            IQBN(64),
            nn.ReLU(),
        ...
        self.block3 = nn.Sequential(
            QConv2D(in_channels=64, out_channels=128, kernel_size=3, stride=2, padding=1),
            IQBN(128),
            nn.ReLU(),
        ...
        self.block4 = nn.Sequential(
            QConv2D(in_channels=128, out_channels=256, kernel_size=3, stride=2, padding=1),
            IQBN(256),
            nn.ReLU(),
            QConv2D(in_channels=256, out_channels=256, kernel_size=3, padding=1),
            IQBN(256),
            nn.ReLU(),
        )
        ...
        self.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(0.3),
            QDense(512, NUM_CLASSES * 4)  # Output 4x classes for quaternion
        )
...
def main():
    ...
    writer = SummaryWriter('runs/QRN34_quat')
